...
public:
    ...
    bool init(const std::string& port = "/dev/ttyS4", int baud = 9600) {
        ...
        fcntl(serial_fd_, F_SETFL, 0);//清除串口文件描述符（serial_fd_）的所有非阻塞 / 异步等文件状态标志，将其恢复为默认的阻塞模式。
    ...
    //     }
    // }
    // 改造后：支持指定运行时长（毫秒），达到时长后自动退出循环
    void autoControlLoop(int16_t motor1, int16_t motor2, int interval_ms = 200, int run_duration_ms = -1) {
        LOG_INFO("CarControl: start auto control loop (interval: " + std::to_string(interval_ms) + 
                "ms, run duration: " + (run_duration_ms >= 0 ? std::to_string(run_duration_ms) + "ms" : "infinite") + ")");
        MotorStatus status;

        // 记录循环开始时间（毫秒级），用于精准计时
        // clock_gettime 精度达纳秒，转换为毫秒用于判断时长
        struct timespec start_time;
        clock_gettime(CLOCK_MONOTONIC, &start_time);
        uint64_t start_ms = start_time.tv_sec * 1000 + start_time.tv_nsec / 1000000;
        ...
        while (true) {
            ...
            if (run_duration_ms >= 0) {
                ...
                // 已运行时长 >= 指定时长，退出循环
                if (current_ms - start_ms >= (uint64_t)run_duration_ms) {
                    LOG_INFO("CarControl: loop finished (run for " + std::to_string(current_ms - start_ms) + "ms)");
                    break;
                }
            }
...
int main() {
    ...
    // 示例1：让小车以motor1=44、motor2=44运行3秒（3000毫秒），循环间隔200ms
    try {
        LOG_INFO("=== 运行3秒 ===");
        driver.autoControlLoop(44, 44, 200, 3000);  
    // 示例3：无限循环（和原来一样）
    // LOG_INFO("=== 无限循环 ===");
    // driver.autoControlLoop(44, 44, 200); // 等价于 driver.autoControlLoop(44, 44, 200, -1)
    } catch (...) {
