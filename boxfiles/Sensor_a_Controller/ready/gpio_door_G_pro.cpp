...
#include <fstream>   //需要接一个变压器，因为门锁需要一个12V的电压
#include <string>    //此代码为通用GPIO版，只需根据说明书套用公式来修改GPIO_PIN的值，
                     //就可以从GPIO4变成GPIO2的调用了
#include <cstdlib>    //只修改了，待验证，但是评估可行度十分高
#include <unistd.h>
#include <sys/stat.h>
#include <sys/types.h>
...
const int GPIO_PIN = 100;                  // IO4对应sysfs编号
const std::string GPIO_SYS_PATH = "/sys/class/gpio/gpio" + std::to_string(GPIO_PIN);
...
int main() {
    ...
    // 2. 自动导出GPIO4（已导出则跳过）
    if (gpioExists()) {
        std::cout << "信息：GPIOx（编号" << GPIO_PIN << "）已导出，直接使用" << std::endl;
    } else {
        std::cout << "信息：正在导出GPIO4（编号" << GPIO_PIN << "）..." << std::endl;
        if (!exportGPIO()) {
            std::cerr << "错误：GPIOx导出失败" << std::endl;
            return 1;
        }
        std::cout << "成功：GPIOx导出完成" << std::endl;
        usleep(200000);  // 等待sysfs节点初始化（200ms）
    }

    // 3. 设置为输出模式
    std::cout << "信息：正在设置GPIOx为输出模式..." << std::endl;
    if (!setGPIOdirection("out")) {
        std::cerr << "错误：设置GPIOx方向失败" << std::endl;
        return 1;
    }
    ...
    std::cout << "  输入 1 → GPIOx输出3.3V（继电器导通，控制端与GND形成电压差）" << std::endl;
    std::cout << "  输入 0 → GPIOx输出0V（继电器断开，控制端与GND无电压差）" << std::endl;
    std::cout << "  输入 q → 退出程序（自动恢复0V断开状态）" << std::endl;
    std::cout << "======================" << std::endl;
    interactiveControl();

    // 7. 程序退出时释放GPIO并恢复0V
    std::cout << "\n信息：正在释放GPIOx并恢复断开状态..." << std::endl;
 ...
 */
void printGpioInfo() {
    std::cout << "  主板接口：IOx" << std::endl;
    std::cout << "  Sysfs编号：" << GPIO_PIN << std::endl;
    std::cout << "  电压域：3.3V" << std::endl;
    std::cout << "  当前状态：" << (getGPIOValue() == 1 ? "3.3V高电平（继电器导通）" : "0V低电平（继电器断开）") << std::endl;
    std::cout << "  接线规范：GPIOx → 1KΩ限流电阻 → 继电器控制端 → 主板GND" << std::endl;
}
