...
InfraredSensor::InfraredSensor(const std::string& deviceId) 
    : PLCComponentDevice(deviceId) {}

bool InfraredSensor::readData() {
    ...
    if (len >= 4 && recvBuf[1] == 0x01) {
        ...
        std::cout << "红外传感器状态：";
        if (state == 0) {
            std::cout << "低电平（正常状态）" << std::endl;
        } else if (state == 1) {
            std::cout << "高电平（触发状态）" << std::endl;
        } else {
            std::cout << "未知状态（值：" << state << "）" << std::endl;
        }
        return true;
    } else {
        std::cout << "红外传感器：响应格式无效" << std::endl;
        return false;
    }
}