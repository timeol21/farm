#include <iostream>      //不符合最新的json的写法
#include <fcntl.h>
#include <unistd.h>
#include <termios.h>
#include <cstring>
...
bool readDeviceStatus(const string& plcId, const string& componentId) {
    if (serialFd < 0) return false;

    const auto& component = globalConfig["plc_devices"][plcId]["components"][componentId];
    vector<unsigned char> sendBuf = hexStringsToBytes(component["commands"]["query"]);
    ...
    cout << "Sent [" << component["name"] << "] query: ";
    ...
    if (len > 0) {
        ...
        if (len >= 4 && recvBuf[1] == 0x01) {
            uint8_t state = recvBuf[3];
            cout << component["name"] << " Status: " << (state == 0 ? "Normal" : "Triggered!") << endl;
        }
    } else if (len == 0) {
        cout << "No response (timeout)" << endl;
    } else {
        perror("read");
    }
...
int main() {
    ...
    int intervalMs = globalConfig.value("polling_interval_ms", 1000);
    cout << "Starting polling every " << intervalMs << "ms..." << endl;

    while (true) {
        readDeviceStatus(plcId, compId);
...
//g++ -o read_water_json read_water_level_json.cpp -I.