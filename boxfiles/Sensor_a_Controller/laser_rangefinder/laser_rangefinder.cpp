...
const int READ_TIMEOUT_MS = 4000;         // 等模块返回数据的最长时间，超过这个时间则，测量超时
const int MEASURE_INTERVAL_MS = 5000;    // 两次测量间隔总时长， 两次测距之间的总周期，必须 > READ_TIMEOUT_MS
...
// 初始化串口
int initSerialPort() {
    int fd = open(SERIAL_PORT, O_RDWR | O_NOCTTY);   // O_RDWR：可读可写,O_NOCTTY：不把串口当控制终端（不会被系统信号打断）
    if (fd == -1) {
    ...
    tty.c_cflag &= ~CRTSCTS;      // 激光模块 没有 RTS/CTS 流控, 关闭流控 = 不会因为没接流控线而收发失败

    tty.c_cflag |= CREAD | CLOCAL;  // CREAD：打开串口接收,CLOCAL：不检查载波检测（本地直连模式）, TTL 直连必须加，否则不工作
    ...
    //读取模式：非阻塞、立即返回
    tty.c_cc[VTIME] = 0;   //VTIME=0：不等待

    tty.c_cc[VMIN] = 0;      //VMIN=0：读到 0 字节也返回

    if (tcsetattr(fd, TCSANOW, &tty) != 0) {     //清空发送 / 接收缓冲区, 避免上电乱码、残留数据干扰
...
int sendSingleMeasurePacket(int fd) {
    ...
    // unsigned char measure_packet[] = {0X58};   //关闭自动连续距离测量 ，X一定要大写
    std::cout << "\n[Sending Single Measure Packet] ( " << sizeof(measure_packet) << " bytes): ";
    ...
    if (bytes_written != sizeof(measure_packet)) {
        std::cerr << "Error: Send failed (" << bytes_written << "/" << sizeof(measure_packet) << " bytes)" << std::endl;
        return -1;
    }
    return 0;
}
...
bool parseDistanceWithoutChecksum(const unsigned char* response, int length, int& distance_mm) {
    if (length < 13 || response[0] != 0xAA || response[3] != 0x22  || response[5] != 0x03) {
    ...
    for (int i = 1; i < length - 1; i++) {   //开头的0xAA，不加在校验和里面
        calc_checksum += response[i];      // calc，发送出去的报文的校验和的十进制
    }

    // 你要的成功打印格式
    std::cout << "✅ Valid Distance: " << distance_mm << " mm (" << distance_mm / 1000.0 << " m) | Checksum: calc="
              << (int)calc_checksum << ", recv=" << (int)response[length - 1] << std::endl;   //recv，模块发回来的校验和的十进制
    return true;
}

int main() {
    if (!grantSerialPortPermission()) return 1;     //先给串口赋权

    int serial_fd = initSerialPort();               //初始化串口
    ...
    unsigned char baud_packet[] = {0x55};                          //激光测距仪的自动匹配波特率的报文
    ...
    std::cout << "Info: Press Ctrl+C to stop | Target rate: " << (double) (1000.0 / MEASURE_INTERVAL_MS) << "Hz" << std::endl;
    ...
    while (true) {
        ...
        // 固定周期，每次测量周期不一定一样，所有计算剩余时间，补全，保证不管距离有多远测量间隔时间都一致
...
// ztl@RK356X:~/program/boxfiles/Sensor_a_Controller/laser_rangefinder$ g++ laser_rangefinder.cpp -o laser_rangefinder
// ztl@RK356X:~/program/boxfiles/Sensor_a_Controller/laser_rangefinder$ sudo ./laser_rangefinder
// === Step 0: Grant serial port permission ===
// Success: Permission granted to /dev/ttyACM0
...
// === Step 3: Stable single measurement (5000ms interval) ===
// Info: Press Ctrl+C to stop | Target rate: 0.2Hz

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 39 00 10 9A 
// ✅ Valid Distance: 11321 mm (11.321 m) | Checksum: calc=154, recv=154

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 34 00 0F 94 
// ✅ Valid Distance: 11316 mm (11.316 m) | Checksum: calc=148, recv=148

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 3C 00 03 90 
// ✅ Valid Distance: 11324 mm (11.324 m) | Checksum: calc=144, recv=144

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 34 00 10 95 
// ✅ Valid Distance: 11316 mm (11.316 m) | Checksum: calc=149, recv=149

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 37 00 0A 92 
// ✅ Valid Distance: 11319 mm (11.319 m) | Checksum: calc=146, recv=146

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 37 00 0F 97 
// ✅ Valid Distance: 11319 mm (11.319 m) | Checksum: calc=151, recv=151
// ^C
...
// === Step 3: Stable single measurement (5000ms interval) ===
// Info: Press Ctrl+C to stop | Target rate: 0Hz

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 28 A2 00 1B 0A 
// ✅ Valid Distance: 10402 mm (10.402 m) | Checksum: calc=10, recv=10

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 28 A6 00 14 07 
// ✅ Valid Distance: 10406 mm (10.406 m) | Checksum: calc=7, recv=7

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 28 BC 00 3D 46 
// ✅ Valid Distance: 10428 mm (10.428 m) | Checksum: calc=70, recv=70

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 27 CD 00 0F 28 
// ✅ Valid Distance: 10189 mm (10.189 m) | Checksum: calc=40, recv=40

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2A 2D 00 12 8E 
// ✅ Valid Distance: 10797 mm (10.797 m) | Checksum: calc=142, recv=142

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 01 00 0B 5D 
// ✅ Valid Distance: 11265 mm (11.265 m) | Checksum: calc=93, recv=93

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2B 39 00 12 9B 
// ✅ Valid Distance: 11065 mm (11.065 m) | Checksum: calc=155, recv=155

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 31 00 0D 8F 
// ✅ Valid Distance: 11313 mm (11.313 m) | Checksum: calc=143, recv=143

// [Sending Single Measure Packet] ( 9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 2C 34 00 17 9C 
// ✅ Valid Distance: 11316 mm (11.316 m) | Checksum: calc=156, recv=156
// ^C
// ztl@RK356X:~/program/boxfiles/Sensor_a_Controller/laser_rangefinder$ 
// ztl@RK356X:~/program/boxfiles/Sensor_a_Controller/laser_rangefinder$ g++ laser_rangefinder.cpp -o laser_rangefinder
// ztl@RK356X:~/program/boxfiles/Sensor_a_Controller/laser_rangefinder$ sudo ./laser_rangefinder
...
// === Step 3: Stable single measurement (10000ms interval) ===
// Info: Press Ctrl+C to stop | Target rate: 0Hz

// [Sending Single Measure Packet] (9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 00 84 01 AC 56 
// ✅ Valid Distance: 132 mm (0.132 m) | Checksum: calc=0, recv=86
// ^C
// ztl@RK356X:~/program/boxfiles/Sensor_a_Controller/laser_rangefinder$ sudo ./laser_rangefinder
// === Step 0: Grant serial port permission ===
// Success: Permission granted to /dev/ttyACM0
...
// [Sending Single Measure Packet] (9 bytes): AA 00 00 20 00 01 00 02 23 
// [Received Response] (13 bytes): AA 00 00 22 00 03 00 00 00 55 02 19 95 
// ✅ Valid Distance: 85 mm (0.085 m) | Checksum: calc=63, recv=149
// ^C
// ztl@RK356X:~/program/boxfiles/Sensor_a_Controller/laser_rangefinder$ 
