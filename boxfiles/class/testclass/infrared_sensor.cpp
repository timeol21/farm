...
// 读取 X2 状态（防拆报警）- 修正：低电平=触发，高电平=未触发
bool readX2Status() {
    if (serialFd < 0) {
    ...
    // X2 对应的 MODBUS 地址：0x0402（功能码0x01，读取单个线圈）
    ...
    // 打印发送的报文
    cout << "Read X2 (防拆报警) command sent: ";
    ...
    if (len < 0) {
        ...
        cout << "X2 (防拆报警): No response received (timeout)" << endl;
        return true; // 超时不算失败，继续轮询
    } else {
        // 打印接收的响应
        cout << "X2 (防拆报警) response (" << len << " bytes): ";
        for (ssize_t i = 0; i < len; ++i)
            printf("%02X ", recvBuf[i]);
        cout << endl;

        // 解析X2状态（修正逻辑：低电平=触发，高电平=未触发）
        if (len >= 4 && recvBuf[1] == 0x01) { // 确保是功能码0x01的有效响应
            uint8_t x2State = recvBuf[3];
            cout << "X2 (防拆报警) Status: ";
            if (x2State == 0) {
                cout << "Low Level (已触发防拆报警)" << endl;
            } else if (x2State == 1) {
                cout << "High Level (未触发防拆)" << endl;
            } else {
                cout << "Unknown (Value: " << (int)x2State << ")" << endl;
            }
        } else {
            cout << "X2 (防拆报警): Invalid response format" << endl;
        }
    }
...
// 读取 X3 状态（人体检测）- 修正：低电平=触发，高电平=未触发
bool readX3Status() {
    if (serialFd < 0) {
        cerr << "Error: Serial port not initialized" << endl;
        return false;
    ...
    // X3 对应的 MODBUS 地址：0x0403（功能码0x01，读取单个线圈）
    ...
    // 打印发送的报文
    cout << "Read X3 (人体检测) command sent: ";
    for (size_t i = 0; i < sizeof(sendBuf); ++i)
        printf("%02X ", sendBuf[i]);
    cout << endl;
    ...
    if (len < 0) {
        ...
        cout << "X3 (人体检测): No response received (timeout)" << endl;
        return true; // 超时不算失败，继续轮询
    } else {
        // 打印接收的响应
        cout << "X3 (人体检测) response (" << len << " bytes): ";
        for (ssize_t i = 0; i < len; ++i)
            printf("%02X ", recvBuf[i]);
        cout << endl;

        // 解析X3状态（修正逻辑：低电平=触发，高电平=未触发）
        if (len >= 4 && recvBuf[1] == 0x01) { // 确保是功能码0x01的有效响应
            uint8_t x3State = recvBuf[3];
            cout << "X3 (人体检测) Status: ";
            if (x3State == 0) {
                cout << "Low Level (已检测到人)" << endl;
            } else if (x3State == 1) {
                cout << "High Level (未检测到人)" << endl;
            } else {
                cout << "Unknown (Value: " << (int)x3State << ")" << endl;
            }
        } else {
            cout << "X3 (人体检测): Invalid response format" << endl;
        }
    }
...
int main() {
    ...
    cout << "=== Infrared Sensor Monitoring Started ===" << endl;
    cout << "Monitored Sensors: X2(防拆报警), X3(人体检测)" << endl;
...
//             cout << "X2 (防拆报警) Status: ";
//             if (x2State == 0) {
//                 cout << "Low Level (未触发防拆)" << endl;
//             } else if (x2State == 1) {
//                 cout << "High Level (已触发防拆报警)" << endl;
//             } else {
//                 cout << "Unknown (Value: " << (int)x2State << ")" << endl;
//             }
//         } else {
...
//             cout << "X3 (人体检测) Status: ";
//             if (x3State == 0) {
//                 cout << "Low Level (未检测到人)" << endl;
//             } else if (x3State == 1) {
//                 cout << "High Level (已检测到人)" << endl;
//             } else {
//                 cout << "Unknown (Value: " << (int)x3State << ")" << endl;
//             }
//         } else {
