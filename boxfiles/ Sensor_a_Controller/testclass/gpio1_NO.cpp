#include <iostream>    //使用于接在GPIO1上的NO的传感器
#include <fstream>
#include <string>
#include <unistd.h>
#include <stdexcept>
...
int main() {
    ...
    try {
        ...
        std::cout << "初始化完成！传感器已独立供电（常开规则：高电平=不导通，低电平=导通）" << std::endl;
        ...
        while (true) {
            ...
            // 常开传感器逻辑：高电平=不导通，低电平=导通（可根据传感器类型修改）
            sensorState = (gpioLevel == 1) ? "不导通" : "导通";
...
/*
明确 GPIO 电平定义：直接显示原始电平值（高电平 = 1，低电平 = 0），避免歧义；
匹配常开传感器逻辑：根据需求设定 “高电平 = 不导通，低电平 = 导通”，与传感器端子特性对应；
*/