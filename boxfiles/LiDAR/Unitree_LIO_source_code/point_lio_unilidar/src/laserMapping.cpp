...
deque<PointCloudXYZI::Ptr> lidar_buffer;   // 存雷达点云
deque<double> time_buffer;
deque<sensor_msgs::Imu::ConstPtr> imu_deque;  // 存 IMU 数据
...
void SigHandle(int sig)   // Ctrl + C 安全退出程序，避免崩溃、数据损坏。
{
    flg_exit = true;
...
inline void dump_lio_state_to_log(FILE *fp)   // 日志与坐标转换工具函数
{
    V3D rot_ang;
    if (!use_imu_as_input)
    {
...
void pointBodyLidarToIMU(PointType const *const pi, PointType *const po)  // 雷达到 IMU 坐标转换
{
...
void lasermap_fov_segment()  // 局部地图窗口管理
{
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
    // IMU回调
    /*
    ROS 收到 IMU 加速度 + 角速度 就跑这里
    校正时间戳
    把 IMU 数据放进 imu_deque 缓存
    线程安全
    定位：所有 IMU 数据的入口。
    */
    publish_count++;
...
bool sync_packages(MeasureGroup &meas)  // 数据同步（雷达 + IMU 时间对齐）
{
...
void map_incremental()   // 把当前帧有效点**添加到全局地图**，用 iKD-Tree 高效存储。
...
void publish_frame_world(const ros::Publisher &pubLaserCloudFullRes)  // 发布拼接好的地图
{ 
    /*
    ROS 结果发布（向外输出定位 + 地图）
    作用：
    发布世界坐标系下的点云地图
    话题：/pointlio/cloud_registered
    可选择保存 PCD 地图文件
    定位：三维地图发布。
    */ 
    if (scan_pub_en)
...
void set_posestamp(T &out)
{
    ...
    作用：
    把滤波后的位姿，赋值给 ROS 消息
    供 publish_odometry / publish_path 使用
    定位：位姿赋值工具。
    */
    if (!use_imu_as_input)
    {
...
void publish_odometry(const ros::Publisher &pubOdomAftMapped)  // 发布位姿（里程计）
{
    /*
    作用（最重要输出）：
    发布实时定位结果（里程计）
...
void publish_path(const ros::Publisher pubPath)  // 发布运动轨迹
{
    /*
    作用：
    发布机器人走过的轨迹
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        /*** initialize the map kdtree ***/   // 3. 第一次：初始化地图
        if (!init_map)
        {
